...
async function importReport(files, folder = "") {
  toast(`Adding ${files.length} file${files.length === 1 ? "" : "s"}…`, { ms: 60000 });
  const r = await media.importFiles(files, { folder });
  const parts = [];
  if (r.added) parts.push(`Added ${r.added} video${r.added === 1 ? "" : "s"}`);
  if (r.skipped) parts.push(`${r.skipped} already in LunaTV`);
  if (r.rejected) parts.push(`${r.rejected} not video`);
  toast(parts.join(" · ") || "Nothing added", { err: !r.added });
}
...
export async function pickFolder() {
  if ("showDirectoryPicker" in window) {
    try {
      ...
      await walk(dir, 0);
      if (files.length) importReport(files, dir.name); else toast("That folder has no files.");
    } catch (e) { if (e.name !== "AbortError") toast("LunaTV couldn’t read that folder.", { err: true }); }
    return;
  }
...
function subline(m, variant) {
  const ep = epLabel(m);
  if (variant === "continue") return [ep, fmtLeft(media.remaining(m))].filter(Boolean).join(" · ") || "In progress";
...
export function seriesCard(s) {
  ...
  const el = h(`<div class="card"><div class="art" data-id="${esc(m.id)}">${artHTML({ ...m, title: s.series })}</div><button class="open" aria-label="Open ${esc(s.series)}"></button>
...
export function mediaRow(m, { onPlay, trail = "", inSeries = false, extraMenu } = {}) {
  ...
  const el = h(`<div class="row tall" style="padding-right:4px">
    <button class="thumb-sm" data-id="${esc(m.id)}" aria-label="Play ${esc(m.title)}">${media.posterCached(m.id) ? `<img alt="" src="${media.posterCached(m.id)}">` : `<span class="ph">${icon(m.category === "tv" ? "tv" : "film")}</span>`}</button>
    <button class="label" style="text-align:left">${esc(label)}<span class="sub">${esc(sub)}</span></button>
    ${trail}<button class="icon-btn plain" data-menu aria-label="Options for ${esc(m.title)}">${icon("more")}</button></div>`);
...
export function openMediaSheet() {
  sheet({ title: "Open Media", groups: [
    [
      { icon: "film", label: "Open Video", run: pickFiles },
      { icon: "grid", label: "Open Multiple Videos", run: pickFiles },
      folderSupported() && { icon: "folderOpen", label: "Open Folder", run: pickFolder },
    ],
    [
      { icon: "youtube", label: "YouTube Link", sub: "watch, youtu.be, Shorts or playlist", run: () => emit("open-youtube-link") },
      { icon: "link", label: "Direct Stream or Video URL", sub: "HLS (.m3u8), MP4, WebM", run: () => emit("open-direct-url") },
      { icon: "live", label: "Add Live TV Source", sub: "M3U, XMLTV, backup", run: () => emit("add-source") },
    ],
  ] });
}
...
export function initLibrary() {
  setRoot("library", {
    build(content) {
      ...
      const head = h(`<div><h1 class="page-title">Library</h1><div class="btn-row"><button class="btn blue">${icon("plus")}Open Video</button>${folderSupported() ? `<button class="btn">${icon("folderOpen")}Open Folder</button>` : ""}</div><div class="btn-row lib-links"><button class="btn sm">${icon("heart")}Favorites</button><button class="btn sm">${icon("history")}History</button><button class="btn sm only-mobile">${icon("playlist")}Playlists</button></div></div>`);
      ...
      const chips = h(`<div class="chips" role="tablist" aria-label="Categories"></div>`);
      const tools = h(`<div class="lib-tools"></div>`);
      tools.append(searchField("Search your library", v => { q = v; render(); }), segmented([["poster", "Poster"], ["grid", "Grid"], ["list", "List"]], view, v => { view = v; db.setSetting("library.view", v); render(); }));
      ...
      async function render() {
        ...
        chips.replaceChildren(...[["all", "All", all.length], ...media.CATEGORIES.map(([k, l]) => [k, l, counts[k]])].filter(([k, , n]) => k === "all" || n).map(([k, l, n]) => {
        ...
        const out = [];
        if (!all.length) {
          const e = h(`<div class="empty big-empty"><img src="assets/branding/lunatv-crescent.webp" alt="" class="empty-mark"><b>No videos yet</b><span>Open a video to begin building your LunaTV library.</span><button class="btn blue">${icon("plus")}Open Video</button></div>`);
        ...
        if (!q && cat === "all") {
          ...
          const s = shelf("Continue Watching", cont, m => mediaCard(m, { variant: "continue" }));
          if (s) out.push(s);
        }
        if (cat === "tv" && !q) {
          const series = media.seriesOf(items);
          const g = h(`<div class="grid"></div>`); series.forEach(s => g.append(seriesCard(s)));
          out.push(h(`<div class="section-label">${series.length} series</div>`), g);
        } else {
          out.push(h(`<div class="section-label">${items.length} ${items.length === 1 ? "video" : "videos"}</div>`));
          const box = h(`<div></div>`);
          if (items.length) renderItems(box, items, view); else box.append(h(`<div class="empty">No matches.</div>`));
          out.push(box);
        }
  ...
  push({
    bare: true, build(content) {
      const render = async () => {
        ...
        if (!cur) { content.replaceChildren(h(`<div class="empty" style="margin-top:80px">This video was removed from LunaTV.</div>`)); return; }
        ...
        const el = h(`<div>
          ...
          <div class="detail-body">
            <h2>${esc(cur.title)}</h2>
            ${prog ? `<p class="resume-line">Resume from ${fmtTime2(cur.position)}</p>` : ""}
            <div class="btn-row">
              ${has ? `<button class="btn primary" data-a="play">${icon("play")}${prog ? "Resume" : "Play"}</button>${prog ? `<button class="btn" data-a="restart">${icon("restart")}Start Over</button>` : ""}`
                : `<button class="btn primary" data-a="relink">${icon("folderOpen")}Locate File</button>`}
              <button class="round${fav ? " on" : ""}" data-a="fav" aria-label="${fav ? "Remove from favorites" : "Add to favorites"}" aria-pressed="${fav}">${icon(fav ? "heartFill" : "heart")}</button>
              <button class="round" data-a="add" aria-label="Add to playlist">${icon("plus")}</button>
            </div>
            ${has ? "" : `<p class="note warn">File needs to be reconnected: this browser no longer holds the video. Title, poster and your place are kept.</p>`}
            <div class="meta">${[media.categoryName(cur.category), fmtDur(cur.duration), resLabel(cur.height) && `<span class="tag">${resLabel(cur.height)}</span>`, (cur.filename.split(".").pop() || "").toUpperCase(), cur.size ? fmtBytes(cur.size) : "", "Added " + new Date(cur.added).toLocaleDateString()].filter(Boolean).join(" · ")}</div>
        ...
        const acts = {
          ...
          fav: async () => { const on = await toggleFavorite({ type: "media", ref: cur.id, title: cur.title, snapshot: { id: cur.id, title: cur.title } }); toast(on ? "Added to Favorites" : "Removed from Favorites"); render(); },
      ...
      const render = async () => {
        ...
        const el = h(`<div>
          ...
          <div class="detail-body">
            <h2>${esc(s.series)}</h2>
            <div class="btn-row">
              <button class="btn primary" data-a="play">${icon("play")}${media.inProgress(up) ? "Resume" : "Play"} ${esc(epLabel(up))}</button>
        ...
        const acts = {
          ...
          more: () => sheet({ title: s.series, groups: [[
            ...
            { icon: "pencil", label: "Rename Series", run: async () => { const n = await ask({ title: "Rename series", value: s.series }); if (n && n !== s.series) { for (const e of s.episodes) await db.put("library", { ...(await db.get("library", e.id)), series: n }); name = n; emit("library-changed"); } } },
          ]] }),
...
export async function mediaMenu(m, { extra } = {}) {
  ...
  sheet({
    ...
    groups: [
      has ? [
        { icon: "play", label: prog ? `Resume · ${fmtLeft(media.remaining(m))}` : "Play", run: () => playLocal([m]) },
        prog && { icon: "restart", label: "Start Over", run: () => playLocal([m], 0, { restart: true }) },
      ] : [{ icon: "folderOpen", label: "Locate File", sub: "File needs to be reconnected", run: () => relinkPicker(m) }],
      [
        { icon: fav ? "heartFill" : "heart", label: fav ? "Remove from Favorites" : "Add to Favorites", run: async () => { const on = await toggleFavorite({ type: "media", ref: m.id, title: m.title, snapshot: { id: m.id, title: m.title } }); toast(on ? "Added to Favorites" : "Removed from Favorites"); } },
        { icon: "playlist", label: "Add to Playlist", run: () => addToPlaylistSheet({ type: "media", ref: m.id, title: m.title }) },
      ],
      [
        { icon: "pencil", label: "Rename", run: async () => { const t = await ask({ title: "Rename", value: m.title }); if (t) { await media.updateMedia(m.id, { title: t }); toast("Renamed"); } } },
        { icon: "grid", label: "Category", sub: media.categoryName(m.category), run: () => categorySheet(m) },
        m.category === "tv" && { icon: "tv", label: "Edit Series Info", run: () => editSeries(m) },
        { icon: "info", label: "File Info", run: () => fileInfo(m) },
      ],
      extra || [],
      [{ icon: "trash", label: "Remove from LunaTV", danger: true, run: async () => {
        if (await confirmBox({ title: `Remove “${m.title}”?`, message: "This removes it from LunaTV and frees the space it uses. The original file on your device isn’t touched.", ok: "Remove", danger: true })) { await media.removeMedia(m.id); toast("Removed from LunaTV"); }
      } }],
    ],
  });
}
function categorySheet(m) {
  sheet({ title: "Category", subtitle: m.title, groups: [media.CATEGORIES.map(([k, l]) => ({ label: l, check: m.category === k, run: async () => {
    if (k === "tv" && m.category !== "tv") return editSeries(m);
    await media.updateMedia(m.id, { category: k, categoryAuto: false }); toast(`Moved to ${l}`);
  } }))] });
}
async function editSeries(m) {
  const r = await form({ title: "Series Info", ok: "Save", fields: [
...
function fileInfo(m) {
  const rows = [["File", m.filename], ["Size", m.size ? fmtBytes(m.size) : ""], ["Type", m.mime], ["Resolution", m.width ? `${m.width} × ${m.height}` : ""], ["Duration", fmtDur(m.duration)], ["Category", media.categoryName(m.category)], ["Added", new Date(m.added).toLocaleString()]].filter(r => r[1]);
...
function relinkPicker(m) {
  ...
  relinkInput.onchange = async () => {
    ...
    toast(r.ok ? (r.warn || "File reconnected") : r.reason, { err: !r.ok, ms: 3500 });
  };
  relinkInput.click();
}
