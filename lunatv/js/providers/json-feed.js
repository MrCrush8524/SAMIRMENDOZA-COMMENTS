...
export function validateFeed(json) {
  ...
  if (!items) throw new Error("The feed must be a JSON object with an \"items\" array.");
  const ok = items.filter(x => x && typeof x === "object" && (x.videoUrl || x.embedUrl) && (x.title || x.id));
  if (!ok.length) throw new Error("No items with a videoUrl or embedUrl were found.");
...
export const jsonProvider = new (class extends Provider {
  constructor() { super({ id: "json", name: "My Feeds", tabs: [{ id: "all", label: "All Feeds" }] }); }
  async page({ page, adult = false }) {
    if (page > 1) return { items: [], hasMore: false };
    const list = (await feeds()).filter(f => !!f.adult === adult);
    if (!list.length) throw new ProviderError("No feeds yet. Add a JSON feed URL or file in Settings › Discover.", { retry: false });
    const out = [];
    for (const f of list) {
      try {
    ...
    if (!out.length) throw new ProviderError("Your feeds didn’t return anything playable right now.");
...
export const directProvider = new (class extends Provider {
  constructor() { super({ id: "direct", name: "Direct Videos" }); }
  async page({ page }) {
    if (page > 1) return { items: [], hasMore: false };
