...
## Master Build Brief 3.1/16.3: chapter_id -> Array[String] of objective
## marker names already found this run, and chapter_id -> the required
## count for that chapter (mirrored from each chapter's ChapterConfig at
## load time so UI/save code has a single generic place to read it,
## rather than a chapter-specific field like chapter3_room_checks per
## new chapter).
var chapter_objectives_found: Dictionary = {}
var chapter_required_counts: Dictionary = {}

## Master Build Brief 4.1: whether the currently-loaded chapter allows
## the random No-Clip Prison event at all - false for Chapter 1, true
## from Chapter 2 onward. Mirrored from each chapter's ChapterConfig at
## load time (same pattern as chapter_required_counts above) so Player.gd
## has a single flag to poll without needing a ChapterConfig reference.
var no_clip_enabled_for_chapter: bool = false

## Transient, not persisted (a save can't happen mid-prison, same
## reasoning as in_backrooms/in_nightmare above): true while a No-Clip
## Prison environment is loaded in place of the real chapter.
var in_noclip_prison: bool = false

func record_chapter_objective(chapter_id: String, marker_name: String) -> void:
	var found: Array = chapter_objectives_found.get(chapter_id, [])
	if not found.has(marker_name):
		found.append(marker_name)
	chapter_objectives_found[chapter_id] = found
...
func new_run(chosen_dreamer: String) -> void:
	...
	chapter_objectives_found.clear()
	chapter_required_counts.clear()
	no_clip_enabled_for_chapter = false
	in_noclip_prison = false
...
func to_dict() -> Dictionary:
	var data := {
		...
		"chapter_objectives_found": chapter_objectives_found,
	}
...
func from_dict(data: Dictionary) -> bool:
	...
	chapter_objectives_found = data.get("chapter_objectives_found", {}).duplicate()
	chapter_required_counts.clear() # Re-populated by each chapter's spawner on load.
	no_clip_enabled_for_chapter = false # Re-set by the loaded chapter itself.
	in_noclip_prison = false
	in_nightmare = false
	nightmare_depth = 0
